...
## graphics class edit
# TODO:
# - Cursor must appear after any zoom. Refresh event?
class BGraph():
...
#              in the axes of originl data
    ...
    def on_pick(self, event):
        ...
        self.pick_text = ax.annotate('(%.5f, %.5f)'%(xdata, ydata),
                           (xdata, ydata), xytext=(3.8*offset, -offset),
...
# Tst = BGraph(1, 2)
# Tst.axs[0].plot(x, y1)
# a = Tst.axs[1].plot(x, y2, picker=5)
# Tst.cursor(1)
# Tst.cursor(0)
# Tst.activ_pick()

# ylim = Tst.axs[0].get_ylim()
# xlim = Tst.axs[0].get_xlim()
# xper = .5
# yper = .1

# xdata = (xlim[-1]+xlim[0])*xper
# print(xdata)
# ydata = ylim[0]-(ylim[-1]-ylim[0])*yper
# # bbox=dict(boxstyle="round", fc="0.8")
# Tst.axs[0].text(xdata, ydata, r'$x$(m)',
#                 fontsize=11,
#                 va="center", ha="center")
# xper = .08
# yper = .5
# xdata = xlim[0]-(xlim[-1]-xlim[0])*xper
# ydata = (ylim[-1]+ylim[0])*yper
# Tst.axs[0].text(xdata, ydata, r'$y$(m)',
